...
class Node:
    def __init__(self, node_id: int, rcv_port: int, send_port: int,tracker_host:int,tracker_port:int):
    ...
    def gatherDataForTracker(self, sock: socket.socket, data: bytes, addr: tuple,endPoint:str):
        ip, dest_port= addr
        segment = UDPSegment(src_port=sock.getsockname()[1],
                             dest_port=dest_port,
                             data=data)
    ...
    def communicate_with_tracker(self, msg, addr, endPoint):
        try:
            if isinstance(msg, bytes):
                msg_data = base64.b64encode(msg).decode('utf-8')
            ...
            response = requests.post(f"{self.tracker_url}/{endPoint}", json=payload)  # Adding timeout
    ...
    def enter_torrent(self):
        ...
        self.gatherDataForTracker(sock=self.send_socket,data=Message.encode(msg),addr=tuple(config.constants.TRACKER_ADDR),endPoint="register")
    ...
    def inform_tracker_periodically(self, interval: int):
        ...
        self.gatherDataForTracker(sock=self.send_socket,
                          data=msg.encode(),
                          addr=tuple(config.constants.TRACKER_ADDR),endPoint="register")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-th', '--tracker_host', type=str, default='192.168.1.52', help='Tracker host address')
    parser.add_argument('-tp', '--tracker_port', type=int, default=8000, help='Tracker port')
